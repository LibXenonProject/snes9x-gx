...
static PTHREAD cache_thread = NULL;
...
{
	lock(&SMBEnv[i]._SMB_mutex);
}

static inline void _SMB_unlock(int i)
{
	unlock(&SMBEnv[i]._SMB_mutex);
}
...
bool smbInitDevice(const char* name, const char *user, const char *password, const char *share, const char *ip)
{
	...
	if(!smbInited)
	{
		for(i=0;i<MAX_SMB_MOUNTED;i++)
		{
			...
			SMBEnv[i]._SMB_mutex = 0;
		}
		
		TR;

		if(cache_thread == NULL)
			cache_thread = thread_create(process_cache_thread,0,0,0);
