...
static s32 smb_recv(s32 s,void *mem,s32 len)
{
	...
	while(len > 0)
	{
		...
		ret=recv(s,mem+readtotal,read,0);
		if(ret>0)
		{
			readtotal+=ret;
			len-=ret;
...
static void clear_network(s32 s,u8 *ptr)
{
	...
	while(true)
	{
		recv(s,ptr,SMB_MAX_NET_READ_SIZE,0);
...
static s32 do_netconnect(SMBHANDLE *handle)
{
	...
	setsockopt(sock,IPPROTO_TCP,TCP_NODELAY,&set,sizeof(set));
	
	// create non blocking socket
	ret = ioctlsocket(sock, FIONBIO, &set);
	if (ret < 0)
	{
		close(sock);
		return ret;
	...
	{
		TR;
		ret = connect(sock,(struct sockaddr*)&handle->server_addr,sizeof(handle->server_addr));
		if(ret==-EISCONN) break;
		t2=mftb();
		usleep(1000);
		//
		if (tb_diff_msec(t2,t1) > CONN_TIMEOUT)
			break;
